const { proto, generateWAMessageFromContent, getContentType, generateWAMessageContent, generateForwardMessageContent, waUploadToServer, downloadContentFromMessage, jidDecode } = require('@whiskeysockets/baileys');
const {
...
msg.sendButton = async (jid, text, options = {}) => {
    let ooki = generateWAMessageFromContent(
      jid,
      {
        viewOnceMessage: {
          message: {
 ...
 return await this.client.relayMessage(ooki.key.remoteJid, ooki.message, {
    messageId: ooki.key.id,
  });
...
msg.runtime = async () => {
 ...
 msg.sendMsg = async(chat, content, opt = {
...
}, type = "text") => {
    {
	    ...
	    case "interactive": {
		const genMessage = await createInteractiveMessage(content);
		    await client.relayMessage(chat, genMessage.message, {
