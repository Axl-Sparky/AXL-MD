...
msg.react = async (txt) => {
  await client.sendMessage(msg.chat, { react: { text: txt, key: msg.key } } )
}

      msg.sendPoll = (jid, name = '', values = [], selectableCount = 1) => { return client.sendMessage(chat, { poll: { name, values }}) } 

msg.poll = async (chat, text, list) => {
